...
Elm.HeroList.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      ...
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,AddHero)]),_U.list([$Html.text("test")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Fetch)]),_U.list([$Html.text("populate")]))
              ,A2($Html.div,_U.list([]),heroes)]));
   });
   var init = {heroes: _U.list([])};
   var Model = function (a) {    return {heroes: a};};
   return _elm.HeroList.values = {_op: _op,Model: Model,init: init,Fetch: Fetch,AddHero: AddHero,update: update,createDiv: createDiv,view: view};
};
Elm.Main = Elm.Main || {};
